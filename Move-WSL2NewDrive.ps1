...
if ($go)
{
    ...
    $selDistro.BasePath = $selDistro.BasePath.replace("\\?\", "")
    ...
    do
    {
        ...
        $raw = [Utils.System.Text.Encoding]::Unicode.GetBytes((wsl -l -v)) | Where-Object { $_ -ne 0 } 
        
        # convert the result into UTF8
        $text = [Utils.System.Text.Encoding]::UTF8.GetString($raw)
